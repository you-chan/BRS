...
static SDL_Surface *ShipWindow[CT_MAXNUM];
...
void DrawCommand()
{
	...
	rect.src.x = gCommand.shot * C_SIZE;
	rect.src.y = 0;
		...
		if(mState == MAIN_COMMAND){
			if(cState == COMMAND_DIR){
				...
				if(key == SDLK_s || key == SDLK_d)
					...
					gCommand.shot = (gCommand.shot - 7) % 2 + 8;
				else if(key == SDLK_x){
					gChara[0].command[nowcommand] = gCommand.shot;
					nowcommand++;
				}
			}
...
void DestroyWindow()
{
	...
	for(i=0; i<CT_MAXNUM; i++)
		SDL_FreeSurface(ShipWindow[CT_MAXNUM]);
